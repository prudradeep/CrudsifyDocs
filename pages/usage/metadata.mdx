...
- deletedAt (default enabled) (see [Soft delete](./softdelete))
  When **modelOptions.timestamps** is enabled, these properties will automatically be populated during CRUD operations. For example, say I create a group with a payload of:
...
- createdBy (default disabled, activated via enableCreatedBy property in configuration)
- updatedBy (default disabled, activated via enableUpdatedBy property in configuration)
- deletedBy (default disabled, activated via enableDeletedBy property in configuration) (see [Soft delete](./softdelete))
- If enabled, these properties will record the id of the user performing the corresponding action.

This assumes that your authentication credentials (req.auth.credentials) will contain either a **user** object with a **dbPrimaryKey** property.
