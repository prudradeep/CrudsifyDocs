...
```bash
npm install https://github.com/prudradeep/crudsify.git#package
```
## Setup
Create a new file index.js and add the following code to set-up an API with crudsify.

```js filename="index.js"
const server = require("crudsify");
server();
```
Create a new file .env for environment variables and add the following:
```bash filename=".env"
...
JWT_SECRET=YourJWTSecretGo35Here
JWT_ALGO=HS256
...
API_DEV_URL=http://localhost:8080
API_UAT_URL=http://localhost:8080
API_PROD_URL=http://localhost:8080

# DB Dialects
# 'mysql' | 'postgres' | 'sqlite' | 'mariadb' | 'mssql' | 'db2' | 'snowflake' | 'oracle'

# DB Vars for development
...
# DB Vars for uat
DB_UAT_USERNAME=username
DB_UAT_PASSWORD=password
DB_UAT_NAME=db_name
DB_UAT_HOST=host
DB_UAT_PORT=port
DB_UAT_DIALECT=dialect

# DB Vars for production
DB_PROD_USERNAME=username
...
```
Update values as per your needs
